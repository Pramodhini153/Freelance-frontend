    ...
    <div className="min-h-screen flex flex-col justify-center items-center bg-gray-100 py-16 px-6">
      ...
      <div className="max-w-7xl mx-auto mt-16 grid grid-cols-2 md:grid-cols-4 gap-10 text-center">
        {[
          ...
          { icon: FaUsers, count: 521, label: "Active Externals" },
          { icon: FaClock, count: 1453, label: "Total Employees" },
          { icon: FaBuilding, count: 32, label: "Departments Involved" },
        ].map(({ icon: Icon, count, label }, index) => (
          <div key={index} className="flex flex-col items-center gap-x-10">
            <Icon className="text-blue-600 text-7xl mb-2" />
